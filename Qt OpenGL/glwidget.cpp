...
void GLWidget::initializeGL()
{
    ...
    QImage texColor= QImage(":/textures/brickDif.png");
    QImage texNormal= QImage(":/textures/bricksNor.png");
...
//responsavel por Ler o objeto do tipo off, que le o numero de vertices, faces para construcao do objeto
void GLWidget::readOFFFile( const QString &fileName)
{
    std::ifstream stream;
    stream.open(fileName.toLatin1(), std::ifstream::in);
    //validar .toAscii / toLatin1
...
// Calcula as normais de cada vertices do objeto
...
// Calcula as tangentes do objetos
...
// Aplicação das iluminações e texturas
void GLWidget::createShaders(unsigned int currentShader)
{
    destroyShaders();
...
// Cria de forma mais otimizada objetos que já estão prontos para serem renderizados
